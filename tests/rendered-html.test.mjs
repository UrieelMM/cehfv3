...
test("ships Firebase setup, rules, indexes, storage and PWA assets", async () => {
  ...
  assert.match(gradeReportPdfSource, /PNG_SIGNATURE\.every/);
  assert.match(gradeReportPdfSource, /let logoAdded = false/);
...
test("ships a configurable virtual exhibition for the mural", async () => {
  ...
  assert.match(muralSource, /Recorrido guiado/);
  assert.match(muralSource, /muralSceneMotion/);
  assert.match(muralSource, /ResizeObserver/);
  assert.doesNotMatch(muralSource, /Reproducir presentación automáticamente/);
  ...
  assert.match(muralContract, /"flip", "wipe", "drift"/);
  assert.match(muralContract, /"archive", "ocean", "festival"/);
  ...
  assert.match(css, /\.mural-guided-controls/);
  assert.match(css, /\.scene-archive/);
